...
def ServoBody(parent, position=[0., 0., 0., 0., 0., 0., 1.], showServo=False):
    ...
    if showServo:
        ...
        visual.createObject("MeshSTLLoader", name="loader", filename= mesh_path + "SG90_servomotor.stl")
...
class ServoMotor(SofaObject):
    ...
    def __init__(self, parent,
                 translation=[0.0, 0.0, 0.0], rotation=[0.0, 0.0, 0.0], scale=[1.0, 1.0, 1.0], showServo=True, showWheel=False, indice=0):
        ...
        baseFrame.createObject("UniformMass")
        baseFrame.createObject("FixedConstraint",indices=[0])
        ...
        ## INVERSE #######################
        # actuator = angle.createChild('actuator')
        # if indice == 0:
        #     actuator.createObject('SlidingActuator', name="SlidingActuator", template='Vec1', direction=[1], indices=0, maxForce='150000', minForce='-150000') #,context=self.actuatedarms[0].ServoMotor.Angle.getLinkPath())
        # elif indice == 1:
        #     actuator.createObject('SlidingActuator', name="SlidingActuator", template='Vec1', direction=[1], indices=0, maxForce='150000', minForce='-150000') #,context=self.actuatedarms[0].ServoMotor.Angle.getLinkPath())
        # else:
        #     actuator.createObject('SlidingActuator', name="SlidingActuator", template='Vec1', direction=[1], indices=0, maxForce='150000', minForce='-150000') #,context=self.actuatedarms[0].ServoMotor.Angle.getLinkPath())
        ...
        #'0 0 0 1 0 0'
        #'0 0 0 '+str(cos(4*math.pi/3))+' 0 '+str(sin(4*math.pi/3))
        #'0 0 0 '+str(cos(2*math.pi/3))+' 0 '+str(sin(2*math.pi/3))
        
        #angle.createObject("FixedConstraint")
        #servobody.createObject("FixedConstraint")
        #servowheel.createObject("FixedConstraint")
